      ...
      <div className="w-full min-w-[850px]">
        <div className="flex bg-gray-100 border-b border-gray-300 p-2">
          {[...Array(14)].map((_, i) => (
            <div
              key={i}
              className={`h-4 bg-gray-300 rounded mx-1 ${
                i < 3 ? "w-[70px]" : "flex-1"
              }`}
            />
          ))}
        ...
        {[1, 2, 3, 4, 5].map((i) => (
          ...
          >
            <div className="w-[70px] h-8 bg-gray-200 rounded mr-2" />
            ...
            <div className="w-[80px] h-6 bg-gray-200 rounded mr-2" />
            <div className="w-[80px] h-6 bg-gray-200 rounded mr-2" />
            <div className="w-[70px] h-6 bg-gray-200 rounded mr-2" />
            <div className="w-[60px] h-6 bg-gray-200 rounded" />
          </div>
        ))}
      </div>
    </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// MAIN COMPONENT — forwardRef for CalBot integration
// ─────────────────────────────────────────────────────────────────────────────
const RepeatabilitySection = forwardRef<
  SectionBotHandle,
  RepeatabilitySectionProps
>(({ jobId, onStepAdded }, ref) => {
  // ── State ──────────────────────────────────────────────────────────────────
  const [loading, setLoading] = useState(false);
  ...
  const [tableData, setTableData] = useState<RepeatabilityRowData[]>([]);
  const [references, setReferences] = useState<UncertaintyReference[]>([]);
  const [specDefaults, setSpecDefaults] = useState<
    Record<string, SpecDefaultValues>
  >({});
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isAddingStep, setIsAddingStep] = useState(false);
  const [customInput, setCustomInput] = useState({
  ...
  const calculateInterpolation = (val: number): number => {
    if (references.length === 0) return 0;
    const lowers = references.filter((r) => r.indicated_torque <= val);
    const uppers = references.filter((r) => r.indicated_torque >= val);
    const lo = lowers.length > 0 ? lowers[lowers.length - 1] : null;
    const hi = uppers.length > 0 ? uppers[0] : null;
    if (!lo && !hi) return 0;
    if (!lo && hi) return Math.abs(hi.error_value);
    if (lo && !hi) return Math.abs(lo.error_value);
    if (lo && hi && lo.indicated_torque === hi.indicated_torque)
      return Math.abs(lo.error_value);
    if (lo && hi) {
      const x1 = lo.indicated_torque,
        y1 = lo.error_value;
      const x2 = hi.indicated_torque,
        y2 = hi.error_value;
      return Math.abs(y1 + ((val - x1) * (y2 - y1)) / (x2 - x1));
    }
    return 0;
  };
  ...
  const recalculateRow = (
    ...
    if (valid.length === 5) {
      ...
      const corrMean = mean - corrStd;
      const dev =
        row.set_torque && row.set_torque !== 0
          ? ((corrMean - row.set_torque) * 100) / row.set_torque
          : null;
      return {
        ...row,
        mean_xr: mean,
    ...
    applyReadings: (data) => {
      ...
      setTableData((prev) => {
        ...
        data.forEach(({ rowIndex, readings }) => {
          ...
          next[rowIndex] = recalculateRow(row);
        });
        return next;
      });
    },
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        const [jobRes, refRes] = await Promise.all([
          api.get<RepeatabilityResponse>(
            ENDPOINTS.HTW_REPEATABILITY.GET(jobId)
          ),
          api.get<UncertaintyReference[]>(
            ENDPOINTS.HTW_REPEATABILITY.REFERENCES
          ),
        ]);

        setReferences(refRes.data);
        if (jobRes.data.defaults) setSpecDefaults(jobRes.data.defaults);

        let currentData: RepeatabilityRowData[] = [];
        if (
          jobRes.data.status === "success" &&
          jobRes.data.results.length > 0
        ) {
          currentData = jobRes.data.results.map((item) => ({
            step_percent: Number(item.step_percent),
            set_pressure: Number(item.set_pressure) || 0,
        ...
        console.error("Failed to load repeatability data", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleToggleStep = async (targetPercent: number) => {
    ...
    const exists = tableData.some(
      (r) => Number(r.step_percent) === targetPercent
    );

    if (exists) {
      setTableData((prev) =>
        prev.filter((r) => Number(r.step_percent) !== targetPercent)
      );
      try {
        setSaveStatus("saving");
        await api.delete("/htw-calculations/repeatability/step", {
      ...
      setTableData((prev) => {
        ...
        return [...prev, newRow].sort(
          (a, b) => a.step_percent - b.step_percent
        );
      });
    }
  };
  ...
  const handleAddCustomStep = async () => {
    ...
    const updatedData = [...tableData, newRow].sort(
      (a, b) => a.step_percent - b.step_percent
    );
    setTableData(updatedData);
        ...
        <table className="w-full min-w-[850px] border-collapse">
          <thead>
            <tr className="bg-gray-100 text-[10px] font-bold text-gray-700 uppercase tracking-tight text-center">
              <th
                rowSpan={3}
                className="border border-gray-300 p-2 w-[70px] bg-gray-100 sticky left-0 z-10"
              >
                Steps %
              </th>
              <th className="border border-gray-300 p-2 w-[80px]">
                Set Pressure
              </th>
              <th className="border border-gray-300 p-2 w-[80px]">
                Set Torque
              ...
              <th
                colSpan={5}
                className="border border-gray-300 p-2 bg-green-50 text-green-800"
              >
                Readings (Master Standard)
              </th>
              <th
                rowSpan={2}
              ...
              <th
                rowSpan={2}
                className="border border-gray-300 p-2 w-[70px]"
              >
                Dev %
              </th>
              <th
                rowSpan={2}
                className="border border-gray-300 p-2 w-[60px]"
              >
                Tol
              </th>
            </tr>
            <tr className="bg-gray-50 text-[9px] font-bold text-gray-600 text-center">
              ...
              {[1, 2, 3, 4, 5].map((i) => (
                <th
                  key={i}
                  className="border border-gray-300 p-1 bg-green-50 w-[70px]"
                >
                  S{i}
                </th>
              ))}
          ...
          <tbody>
            {tableData.length === 0 ? (
              <tr>
                <td
                  colSpan={14}
                  className="p-6 text-center text-gray-400 italic border border-gray-300"
                >
              ...
              tableData.map((row, rowIndex) => (
                ...
                >
                  ...
                  {row.readings.map((reading, rIndex) => (
                    <td
                      key={rIndex}
                      className="p-0 border border-gray-300 relative bg-white"
                    >
                      <input
                        type="text"
                        value={reading}
                  ...
                  <td className="p-2 border border-gray-300 text-center text-gray-600 text-sm">
                    {row.corrected_standard !== null
                      ? row.corrected_standard.toFixed(2)
                      : "-"}
                  </td>
                  <td className="p-2 border border-gray-300 text-center text-gray-600 text-sm">
                    {row.corrected_mean !== null
                      ? row.corrected_mean.toFixed(2)
                      : "-"}
                  </td>
                  ...
                  {(() => {
                    ...
                    return (
                      <td
                        className={`p-2 border border-gray-300 text-center font-bold text-sm ${
                          isEmpty
                            ? "text-gray-400"
                            : isFail
...
RepeatabilitySection.displayName = "RepeatabilitySection";
export default RepeatabilitySection;