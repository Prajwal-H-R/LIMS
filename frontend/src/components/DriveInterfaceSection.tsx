...
>(({ jobId }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    /** Expose rows: use set_torque from meta, 10 readings each */
    getRows: () =>
      tableData.map((row, rowIndex) => ({
        // Drive interface uses set_torque from meta (same for all rows)
        set_torque: meta.set_torque,
        readingCount: 10,
        rowIndex,
      })),

    /** Bot injects readings — recalculate mean + b_int instantly */
    applyReadings: (data) => {
      hasUserEdited.current = true;
      setSaveStatus("idle");
      setTableData((prev) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setLoading(true);
      try {
        const res = await api.get<DriveInterfaceResponse>(
          `${ENDPOINTS.HTW_CALCULATIONS.DRIVE_INTERFACE}/${jobId}`
        );
  ...
  return (
    <div className="flex flex-col w-full animate-in fade-in duration-500 bg-white border border-gray-200 rounded-xl shadow-sm p-4 mt-6">
      ...
      <div className="mb-4 flex justify-between items-center">
        <h2 className="text-sm font-bold text-black uppercase tracking-tight border-l-4 border-indigo-500 pl-2">
          D. Variation due to geometric effect of the drive interface (b
          <sub>int</sub>)
        </h2>
