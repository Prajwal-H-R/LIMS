...
import LoadingPointSection from "../components/LoadingPointSection";
import { useRecordLock } from "../hooks/useRecordLock";
import CalibrationAIBot, {
  SectionBotHandle,
} from "../components/CallibrarionAIBot";
...
const customStyles = `
  ...
  .animate-step { animation: fadeInSlideUp 0.4s ease-out forwards; }
  .ribbon-step {
    position: relative;
    clip-path: polygon(0 0, calc(100% - 14px) 0, 100% 50%, calc(100% - 14px) 100%, 0 100%, 14px 50%);
    transition: all 0.3s ease-in-out;
  ...
  .ribbon-container { filter: drop-shadow(0 4px 6px rgba(0,0,0,0.05)); }
  @keyframes softPulse {
    0%   { filter: brightness(1); }
    50%  { filter: brightness(1.1); }
    100% { filter: brightness(1); }
  }
  .active-ribbon { animation: softPulse 2s infinite; z-index: 10; }
`;
        ...
        <div className="p-6">
          <div className="flex items-start gap-4 mb-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-500 mt-1">
                The system cannot create this job because the following
                standards are empty.
              </p>
            </div>
            <button
          ...
          <div className="bg-gray-50 rounded-lg border border-gray-200 p-4 mb-6 max-h-60 overflow-y-auto">
            ...
            <ul className="space-y-2">
              {missingTables.map((table, index) => (
                <li
                  key={index}
                  className="flex items-center gap-2 text-sm text-red-700 font-mono bg-red-50/50 px-2 py-1 rounded border border-red-100"
                >
                  <span className="w-1.5 h-1.5 bg-red-500 rounded-full" />
                  {table}
                </li>
...
const EnvironmentCheckSection: React.FC<{
  ...
  useEffect(() => {
    const loadData = async () => {
      let existingRecord: EnvironmentRecord | null = null;
      try {
        const res = await api.get<EnvironmentRecord[]>(
          `/staff/jobs/${jobId}/environment?condition_stage=${stage}`
        );
        if (res.data && res.data.length > 0) {
          existingRecord = res.data[0];
          setRecord(existingRecord);
      ...
      try {
        if (existingRecord?.data.environment_config_id) {
          const histRes = await api.get(
            ENDPOINTS.HTW_ENVIRONMENT_CONFIG.GET(
              existingRecord.data.environment_config_id
            )
          );
          setConfigLimits(histRes.data);
        } else {
          const activeRes = await api.get(
            ENDPOINTS.HTW_ENVIRONMENT_CONFIG.ACTIVE
          );
          setConfigLimits(activeRes.data);
        }
      } catch (configErr) {
        console.error("Failed to load config limits", configErr);
      }
    };
    if (jobId) loadData();
  }, [jobId, stage]);
  ...
  const isTempValid =
    temp !== "" &&
    parseFloat(temp) >= tempMin &&
    parseFloat(temp) <= tempMax;
  const isHumValid =
    humidity !== "" &&
    parseFloat(humidity) >= humMin &&
    parseFloat(humidity) <= humMax;
  const isReadyToSubmit = temp !== "" && humidity !== "";
  const isReadOnly = (!!record && record.validation.is_valid) || isLocked;
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.post<EnvironmentRecord>(
        `/staff/jobs/${jobId}/environment`,
        {
          condition_stage: stage,
          ambient_temperature: parseFloat(temp),
          temperature_unit: "°C",
      ...
      if (err.response?.status === 422) {
        ...
        if (detail?.validation?.warnings) {
          setError(
            "Values out of range: " + detail.validation.warnings.join(", ")
          );
        } else {
          setError(
            `Values must be: Temp ${tempMin}-${tempMax}°C, Humidity ${humMin}-${humMax}%`
          );
        }
      } else {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-6 animate-step">
      <div
        className={`w-full max-w-3xl bg-white rounded-2xl shadow-xl border overflow-hidden transition-all duration-300 ${
          error
      ...
      >
        <div
          className={`px-8 py-6 border-b flex items-start gap-4 ${
            isReadOnly && !isLocked
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div
              className={`group relative p-4 rounded-xl border-2 transition-all duration-200 ${
                isReadOnly
                  ...
                  : "border-gray-100 bg-white focus-within:border-blue-300 focus-within:shadow-md"
              }`}
            >
              <div className="flex justify-between items-center mb-2">
              ...
              <div className="flex items-center gap-3">
                <Thermometer
                  className={`h-6 w-6 ${
                    isTempValid ? "text-green-500" : "text-gray-400"
                  }`}
                />
                <input
                  type="number"
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <Droplets
                  className={`h-6 w-6 ${
                    isHumValid ? "text-green-500" : "text-gray-400"
                  }`}
                />
                <input
                  type="number"
          ...
          <div className="mt-8 flex justify-center">
            {!isReadOnly ? (
              <button
                onClick={handleSave}
                disabled={loading || !isReadyToSubmit || isLocked}
                className={`relative px-8 py-3 rounded-lg font-bold text-sm shadow-md transition-all duration-300 flex items-center gap-2 ${
                  isReadyToSubmit && !isLocked
...
// ─────────────────────────────────────────────────────────────────────────────
const CalibrationPage: React.FC = () => {
  const { inwardId, equipmentId } = useParams<{
    inwardId: string;
    equipmentId: string;
  }>();
  const navigate = useNavigate();
  const location = useLocation();
  const { isLocked, lockedBy } = useRecordLock(
    "EQUIPMENT",
    equipmentId ? Number(equipmentId) : null
  );

  // ── Bot refs — one per calibration section ─────────────────────────────────
  const repeatabilityRef = useRef<SectionBotHandle>(null);
  const reproducibilityRef = useRef<SectionBotHandle>(null);
  const outputDriveRef = useRef<SectionBotHandle>(null);
  const driveInterfaceRef = useRef<SectionBotHandle>(null);
  const loadingPointRef = useRef<SectionBotHandle>(null);
  ...
  const [showTerminateModal, setShowTerminateModal] = useState(false);
  const [terminating, setTerminating] = useState(false);
  const [showManualDeviationModal, setShowManualDeviationModal] =
    useState(false);
  const [creatingManualDeviation, setCreatingManualDeviation] = useState(false);
  const [manualDeviationRemarks, setManualDeviationRemarks] = useState("");
  const [manualDeviationFiles, setManualDeviationFiles] = useState<File[]>([]);
  const [manualDeviationUploadProgress, setManualDeviationUploadProgress] =
    useState(0);
  const [manualDeviationUploadStage, setManualDeviationUploadStage] =
    useState("");

  const [showMissingStandardsModal, setShowMissingStandardsModal] =
    useState(false);
  const [missingStandardsList, setMissingStandardsList] = useState<string[]>(
    []
  );
  ...
  const [manufacturerSpec, setManufacturerSpec] =
    useState<HTWManufacturerSpec | null>(null);
  const [allResolutions, setAllResolutions] = useState<
    HTWPressureGaugeResolution[]
  >([]);

  const [masterStandardInputs, setMasterStandardInputs] =
    useState<MasterStandardInput>({
      standard1: {},
      standard2: {},
      standard3: {},
    });
  ...
  const fetchSavedStandards = async (currentJobId: number) => {
    try {
      const response = await api.get<BackendStandardsResponse>(
        `/jobs/${currentJobId}/auto-selected-standards`
      );
      const { exists, standards } = response.data;
      if (exists && standards.length > 0) {
        const s1 = standards.find((s) => s.standard_order === 1);
        ...
        const mapSnapshot = (
          snap: HTWJobStandardSnapshot | undefined
        ): Partial<HTWMasterStandard> => {
          if (!snap) return {};
          return {
            id: snap.master_standard_id,
            nomenclature: snap.nomenclature,
        ...
        setMasterStandardInputs({
          standard1: mapSnapshot(s1),
          standard2: mapSnapshot(s2),
          standard3: mapSnapshot(s3),
        });
        setIsWorksheetSaved(true);
      }
    } catch (e) {
  ...
  const handleRefreshStandards = async () => {
    ...
    try {
      ...
      console.error("Failed to auto-update standards", err);
    }
  };
  ...
  useEffect(() => {
    const initData = async () => {
      ...
      try {
        const [inwardRes, resolutionsRes] = await Promise.all([
          api.get<InwardDetailResponse>(
            `${ENDPOINTS.STAFF.INWARDS}/${inwardId}`
          ),
          api.get<HTWPressureGaugeResolution[]>(
            `${ENDPOINTS.HTW_PRESSURE_GAUGE_RESOLUTIONS.LIST}`
          ),
        ]);

        setAllResolutions(resolutionsRes.data);
        const fetchedResolutions = resolutionsRes.data;
        const defaultResUnit =
          fetchedResolutions.length > 0 ? fetchedResolutions[0].unit : "";
        const defaultPressure =
          fetchedResolutions.length > 0
            ? String(fetchedResolutions[0].pressure)
            : "";
        ...
        let existingJob: HTWJobResponse | null = null;
        try {
          const jobRes = await api.get<any>(
            `/htw-jobs/?inward_eqp_id=${equipmentId}`
          );
          const jobData = jobRes.data;
        ...
        if (existingJob) {
          ...
          try {
            const preRes = await api.get(
              `/staff/jobs/${existingJob.job_id}/environment/pre-status`
            );
            if (preRes.data.pre_is_valid) {
              setPreEnvValid(true);
              const postRes = await api.get(
                `/staff/jobs/${existingJob.job_id}/environment/post-status`
              );
              if (postRes.data.post_is_valid) {
                setPostEnvValid(true);
                setActiveTab("POST");
          ...
          setDeviceDetails({
            calibrationDate:
              existingJob.calibration_date ||
              new Date().toISOString().split("T")[0],
            materialNomenclature:
              existingJob.material_nomenclature ||
              foundEquipment.material_description ||
              "",
            make: existingJob.make || foundEquipment.make || "",
            model: existingJob.model || foundEquipment.model || "",
            serialNo:
              existingJob.serial_no || foundEquipment.serial_no || "",
            range:
              existingJob.range_value || foundEquipment.range || "",
            rangeUnit: existingJob.range_unit || "",
            calibrationMode:
              existingJob.calibration_mode || "Clockwise",
            type: existingJob.device_type || "Indicating",
            classification:
              existingJob.classification || "Type I Class C",
            resolutionOfPressureGauge: existingJob.resolution_pressure_gauge
              ? String(existingJob.resolution_pressure_gauge)
              : defaultPressure,
            resolutionOfPressureGaugeUnit:
              existingJob.resolution_unit || defaultResUnit,
          });
        } else {
          let extractedRangeUnit = foundEquipment.range_unit || "";
          if (!extractedRangeUnit && foundEquipment.range) {
  ...
  useEffect(() => {
    const fetchManufacturerSpec = async () => {
      ...
      try {
        const res = await api.get<HTWManufacturerSpec[]>(
          `${ENDPOINTS.HTW_MANUFACTURER_SPECS.LIST}?is_active=true`
        );
        const match = res.data.find(
          (spec) =>
            spec.make?.toLowerCase().trim() ===
              deviceDetails.make.toLowerCase().trim() &&
            spec.model?.toLowerCase().trim() ===
              deviceDetails.model.toLowerCase().trim()
        );
        setManufacturerSpec(match || null);
      } catch {
        setManufacturerSpec(null);
      }
    };
  ...
  useEffect(() => {
    ...
    setDeviceDetails((prev) => {
      ...
      if (!prev.rangeUnit || prev.rangeUnit.trim() === "") {
        const isHTW = prev.materialNomenclature
          .toUpperCase()
          .includes("HYDRAULIC TORQUE WRENCH");
        const unitToUse = isHTW
          ? manufacturerSpec.torque_unit
          : manufacturerSpec.pressure_unit;
        if (unitToUse) {
          next.rangeUnit = unitToUse;
          changed = true;
        }
      }

      if (manufacturerSpec.pressure_unit) {
        const pu = manufacturerSpec.pressure_unit;
        if (prev.resolutionOfPressureGaugeUnit !== pu) {
          const matchedRes = allResolutions.find(
            (r) => r.unit.toLowerCase() === pu.toLowerCase()
          );
          if (matchedRes) {
            next.resolutionOfPressureGaugeUnit = matchedRes.unit;
            next.resolutionOfPressureGauge = String(matchedRes.pressure);
            changed = true;
          }
        }
      }
    ...
    manufacturerSpec,
    deviceDetails.materialNomenclature,
    allResolutions,
    loading,
    isLocked,
  ]);
  ...
  // ── Handlers ───────────────────────────────────────────────────────────────
  const handleDeviceDetailChange = (
    field: keyof typeof deviceDetails,
    value: string
  ) => {
    if (isLocked) return;
    setDeviceDetails((prev) => ({ ...prev, [field]: value }));
  ...
  const handleResolutionUnitChange = (
    e: React.ChangeEvent<HTMLSelectElement>
  ) => {
    if (isLocked) return;
    const selectedUnit = e.target.value;
    const matched = allResolutions.find((r) => r.unit === selectedUnit);
    setDeviceDetails((prev) => ({
      ...prev,
      resolutionOfPressureGaugeUnit: selectedUnit,
      resolutionOfPressureGauge: matched
        ? String(matched.pressure)
        : prev.resolutionOfPressureGauge,
    }));
  };
  ...
  const handleValidateAndCreate = async () => {
    ...
    try {
      ...
      const res = await api.post<HTWJobResponse>(
        ENDPOINTS.HTW_JOBS.CREATE,
        payload
      );
      const newJobId = res.data.job_id;
      setJobId(newJobId);
      ...
      if (
        ...
        err.response.data?.detail?.error_code === "MISSING_STANDARDS"
      ) {
        setMissingStandardsList(
          err.response.data.detail.missing_tables || []
        );
        setShowMissingStandardsModal(true);
      } else {
        const msg =
  ...
  const handleSaveWorksheet = async () => {
    ...
    try {
      ...
      const response = await api.put(
        `/jobs/${jobId}/auto-select-standards`,
        null,
        {
          params: {
            inward_eqp_id: Number(equipmentId),
      ...
      if (
        response.status === 200 ||
        response.data.status === "recomputed"
      ) {
        await fetchSavedStandards(jobId);
        setIsWorksheetSaved(true);
      } else {
  ...
  const handleFinishAndExit = async () => {
    ...
    try {
      const finishResponse = await api.post(ENDPOINTS.HTW_JOBS.FINISH(jobId));
      const finalStatus =
        finishResponse.data?.final_job_status || "Completed";
      ...
      setJobStatus(finalStatus);
      const targetTab = "completed";

      if (finalStatus.toLowerCase().includes("oot")) {
        alert(
          "Job finalized. Out-of-Tolerance detected. A deviation record has been created."
        );
      } else {
        alert("Job finalized successfully and is within tolerance.");
      }
      ...
      const detail =
        ...
        "Failed to finalize the job. Please check server logs.";
      alert(`Error: ${detail}`);
    } finally {
      setFinishing(false);
    }
  ...
  const handleTerminateConfirm = async () => {
    ...
    try {
      ...
      alert(
        `Error: ${err.response?.data?.detail || "Failed to terminate job"}`
      );
    } finally {
  ...
  const handleManualDeviationCreate = async () => {
    ...
    try {
      ...
      setManualDeviationUploadProgress(
        manualDeviationFiles.length > 0 ? 10 : 100
      );
      const createdDeviationId = createRes?.data?.deviation_id;
        ...
        await api.post(
          ...
          formData,
          {
            ...
            onUploadProgress: (progressEvent) => {
              ...
              const pct = Math.round((progressEvent.loaded * 100) / total);
              setManualDeviationUploadProgress(
                Math.min(100, 10 + Math.round((pct * 90) / 100))
              );
            },
          }
        );
      ...
      setShowManualDeviationModal(false);
      alert(
        "Manual deviation created successfully. Job status moved to On Hold."
      );
    } catch (err: any) {
      alert(
        `Error: ${
          err.response?.data?.detail ||
          "Failed to create manual deviation."
        }`
      );
    } finally {
      setManualDeviationUploadStage("");
  ...
  const goToNextStep = () => {
    ...
    if (activeTab === "PRE" && !preEnvValid) {
      alert("Please validate the Pre-Check environment conditions.");
      return;
    }
  ...
  // ── Style helpers ──────────────────────────────────────────────────────────
  const labelClass =
    "text-[11px] font-bold text-gray-500 uppercase tracking-wider mb-0.5 ml-0.5";
  const wrapperClass = "flex flex-col";
  const inputBase =
    "w-full p-2 text-sm border rounded-lg focus:outline-none shadow-sm transition-all";
  const editableInput = `${inputBase} bg-white border-gray-300 text-gray-900 font-medium focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-100 disabled:text-gray-500`;
  const readOnlyInput = `${inputBase} bg-gray-100 text-gray-600 border-gray-300 font-medium cursor-not-allowed`;
  const groupLeftReadOnly =
    "w-2/3 p-2 text-sm bg-gray-100 border border-gray-300 border-r-0 rounded-l-lg text-gray-600 cursor-not-allowed focus:outline-none";
  const groupRightReadOnly =
    "w-1/3 p-2 text-sm bg-gray-100 border border-gray-300 rounded-r-lg text-gray-600 font-bold text-center cursor-not-allowed focus:outline-none";
  const groupContainer = "flex shadow-sm rounded-lg overflow-hidden";
  const groupLeft =
    "w-2/3 p-2 text-sm bg-gray-50 border border-gray-300 border-r-0 rounded-l-lg text-gray-900 focus:outline-none disabled:bg-gray-100";
  const groupRightSelect =
    "w-1/3 p-2 text-sm bg-white border border-gray-300 rounded-r-lg text-gray-900 font-medium focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none disabled:bg-gray-100";
  const formOpacity = isLocked
  ...
  if (error)
    return (
      <div className="bg-white h-screen flex flex-col justify-center items-center p-8 text-center">
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl border border-gray-100 h-screen flex flex-col overflow-hidden relative">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex flex-col">
            ...
            <p className="text-gray-500 text-xs mt-1">
              HTW Calibration Process
            </p>
            <div className="mt-2 inline-flex items-center">
              <span className="bg-blue-100 text-blue-800 text-[10px] font-bold px-2 py-0.5 rounded border border-blue-200 mr-2">
                NEPL ID
              </span>
              <span className="text-base font-mono font-bold text-gray-800 tracking-tight">
                {equipment.nepl_id}
              </span>
              {isValidated && (
                <span className="ml-3 flex items-center gap-1 text-green-600 bg-green-50 px-2 py-0.5 rounded-full text-[10px] font-bold border border-green-100">
                  <CheckCircle className="h-3 w-3" /> JOB CREATED
                </span>
              )}
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={handleBack}
              className="flex items-center gap-2 px-4 py-2 bg-gray-200 rounded-lg hover:bg-gray-300 text-sm font-semibold text-gray-700 transition-colors"
            >
              <ArrowLeft size={18} /> Exit Job
            </button>
          </div>
        </div>
      </div>
      ...
      {/* Body */}
      <div
        className={`flex-1 overflow-y-auto px-8 py-6 bg-white ${formOpacity}`}
      >
        <div className="mb-6">
          <h2 className="text-sm font-bold text-gray-800 flex items-center gap-2 border-l-4 border-blue-500 pl-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-x-12 gap-y-5 pb-8 border-b border-gray-100">
          <div className={wrapperClass}>
            <label className={labelClass}>
              <span className="flex items-center gap-1">
                <Calendar className="h-3 w-3" /> Calibration Date
              </span>
            </label>
            <input
              type="date"
              value={deviceDetails.calibrationDate}
              onChange={(e) =>
                handleDeviceDetailChange("calibrationDate", e.target.value)
              }
          ...
          <div className={wrapperClass}>
            <label className={labelClass}>Material Nomenclature</label>
            <input
              type="text"
              value={deviceDetails.materialNomenclature}
              readOnly
          ...
          <div className={wrapperClass}>
            ...
            <select
              ...
              onChange={(e) =>
                handleDeviceDetailChange("type", e.target.value)
              }
              className={editableInput}
              disabled={isLocked}
            >
          ...
          <div className={wrapperClass}>
            ...
            <input
              type="text"
              value={deviceDetails.classification}
              onChange={(e) =>
                handleDeviceDetailChange("classification", e.target.value)
              }
              className={editableInput}
              disabled={isLocked}
            />
          ...
          <div className={wrapperClass}>
            ...
            <div className={groupContainer}>
              <input
                ...
                readOnly
                placeholder="Min - Max"
                className={groupLeftReadOnly}
              />
              <input
                type="text"
                value={deviceDetails.rangeUnit}
                readOnly
                placeholder="Unit"
                className={groupRightReadOnly}
              />
            </div>
          </div>
          <div className={wrapperClass}>
            <label className={labelClass}>Resolution of Pressure Gauge</label>
            <div className={groupContainer}>
              <input
                type="text"
                value={deviceDetails.resolutionOfPressureGauge}
                readOnly
                placeholder="Auto-filled"
                className={groupLeft}
              />
              <select
                value={deviceDetails.resolutionOfPressureGaugeUnit}
                onChange={handleResolutionUnitChange}
                className={groupRightSelect}
                disabled={isLocked}
              >
                <option value="">Select Unit</option>
                {allResolutions.map((res, index) => (
                  <option key={`${res.unit}-${index}`} value={res.unit}>
                    {res.unit}
                  </option>
                ))}
              </select>
          ...
          <div className={wrapperClass}>
            <label className={labelClass}>Calibration Mode</label>
            <select
              value={deviceDetails.calibrationMode}
              onChange={(e) =>
                handleDeviceDetailChange("calibrationMode", e.target.value)
              }
              className={editableInput}
              disabled={isLocked}
            >
        ...
        {!isValidated && (
          <div className="flex justify-end pt-6 pb-6">
            <button
              ...
              className="px-6 py-2.5 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition-colors shadow-sm disabled:opacity-70 flex items-center gap-2 disabled:cursor-not-allowed"
            >
              {validating ? (
          ...
          <div className="mt-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                    <thead>
                      <tr className="bg-gray-50 text-gray-600 text-[11px] uppercase tracking-wider border-b border-gray-200">
                        <th className="px-4 py-3 font-bold w-1/5">Field</th>
                        {["standard1", "standard2", "standard3"].map(
                          (stdKey, idx) => (
                            <th
                              key={stdKey}
                              className="px-4 py-3 font-bold w-1/4 min-w-[200px]"
                            >
                    ...
                    <tbody className="divide-y divide-gray-100 text-sm">
                      {[
                        ...
                        },
                        {
                          ...
                          field: "calibration_valid_upto",
                          type: "date",
                        },
                        {
                          label: "Resolution Of Transducer",
                          field: "resolution",
                          type: "group",
                        },
                        {
                          label: "Traceability",
                          field: "traceable_to_lab",
                          type: "textarea",
                        },
                        ...
                        >
                          <td className="px-4 py-3 font-medium text-gray-700 text-xs uppercase bg-gray-50/20 align-top pt-4">
                            {label}
                          </td>
                          {(
                            ["standard1", "standard2", "standard3"] as const
                          ).map((stdKey) => {
                            const std = masterStandardInputs[stdKey];
                            return (
                              <td key={stdKey} className="px-4 py-2">
                                {type === "textarea" && (
                                  <textarea
                                    value={(std as any)[field] || ""}
                                    readOnly
                                    className={`${readOnlyInput} min-h-[50px] resize-none`}
                                    rows={2}
                                    placeholder="-"
                                  />
                                )}
            ...
            <div>
              {isWorksheetSaved ? (
                <div className="bg-gray-50 border border-gray-200 rounded-xl p-6">
                  ...
                  <div className="ribbon-container flex w-full mb-8">
                    {STEPS.map((step, index) => {
                      ...
                      const isCompleted = index < currentIndex;
                      let bgClass =
                        "bg-white border-b-2 border-gray-200 text-gray-400";
                      if (isCompleted)
                        bgClass =
                          "bg-gradient-to-r from-emerald-500 to-teal-500 text-white shadow-inner";
                      if (isActive)
                        bgClass =
                          "active-ribbon bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg";

                      return (
                        <div
                          key={step.id}
                          className={`ribbon-step flex items-center justify-center flex-1 h-12 text-[10px] md:text-xs font-bold uppercase tracking-wider select-none ${bgClass} ${
                            index !== 0 ? "-ml-3" : ""
                          } z-${30 - index}`}
                        >
                          <div className="flex items-center gap-2 transform -skew-x-0">
                  ...
                  <div className="mt-8 min-h-[400px]">
                    ...
                    {/* ── Each section gets its bot ref ── */}
                    {activeTab === "A" && (
                      <RepeatabilitySection
                        ref={repeatabilityRef}
                        jobId={jobId}
                    ...
                    {activeTab === "D" && (
                      <DriveInterfaceSection
                        ref={driveInterfaceRef}
                        jobId={jobId}
                      />
                    )}
                    {activeTab === "E" && (
                ...
                <div className="border-2 border-dashed border-gray-200 rounded-xl bg-gray-50 flex flex-col items-center justify-center text-center p-12">
                  ...
                  <p className="text-gray-500 text-sm max-w-[300px]">
                    Please confirm and save the master standards above to begin
      ...
      {/* Footer Navigation */}
      <div
        className={`flex-none px-8 py-5 border-t border-gray-100 flex justify-end gap-4 bg-gray-50 ${
        ...
        {isValidated && isWorksheetSaved ? (
          <div className="flex w-full justify-between">
            <button
              ...
              disabled={activeTab === "PRE" || isLocked}
              className="px-5 py-2 text-sm bg-white text-gray-700 font-medium rounded-lg hover:bg-gray-100 border border-gray-300 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
            >
            ...
            {activeTab === "POST" ? (
              ...
              <button
                ...
                disabled={isLocked}
                className="px-6 py-2 text-sm bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition-colors shadow-md flex items-center gap-2"
              >
          ...
          <button
            className="px-5 py-2 text-sm bg-gray-300 text-white font-medium rounded-lg cursor-not-allowed flex items-center gap-2"
            disabled
          >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="p-6">
              ...
              <p className="text-gray-600 mb-6">
                Are you sure you want to terminate this calibration job? The
                status will be marked as{" "}
                <span className="font-bold text-red-600">"Terminated"</span>{" "}
                and you will be returned to the job list.
              </p>
              <div className="flex justify-end gap-3">
                <button
            ...
            <div className="p-6">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Add remarks and optional attachments to place this job On
                    Hold.
                  </p>
                </div>
              </div>
              <div className="space-y-4">
                <div>
                  ...
                  <textarea
                    ...
                    value={manualDeviationRemarks}
                    onChange={(e) =>
                      setManualDeviationRemarks(e.target.value)
                    }
                  />
                </div>
                <div>
                  ...
                  {manualDeviationFiles.length > 0 && (
                    <p className="text-xs text-gray-500 mt-1">
                      {manualDeviationFiles.length} file(s) selected
                    </p>
                  )}
                </div>
                {creatingManualDeviation && (
                  <div>
                    <div className="flex items-center justify-between text-xs text-gray-600 mb-1">
                      <span>
                        {manualDeviationUploadStage || "Uploading..."}
                      </span>
                      <span>{manualDeviationUploadProgress}%</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2.5 overflow-hidden">
                      <div
                        className="h-2.5 rounded-full bg-amber-600 transition-all duration-200"
                        style={{
                          width: `${manualDeviationUploadProgress}%`,
                        }}
                      />
                    </div>
                  </div>
      ...
      {/* Missing Standards Modal */}
      <MissingStandardsModal
        isOpen={showMissingStandardsModal}
        missingTables={missingStandardsList}
        onClose={() => setShowMissingStandardsModal(false)}
      />

      {/* ── Cal-Bot AI — renders as floating button, dev-only ── */}
      <CalibrationAIBot
        activeStepId={activeTab}
        isVisible={isValidated && !!jobId}
        sectionRefs={{
          repeatability: repeatabilityRef,
          reproducibility: reproducibilityRef,
          outputDrive: outputDriveRef,
          driveInterface: driveInterfaceRef,
          loadingPoint: loadingPointRef,
        }}
      />
    </div>
  );
