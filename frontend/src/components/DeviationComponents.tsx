...
// ============================================================
// SRF LIST PAGE - Shows grouped SRFs with equipment count
// ============================================================
export const DeviationPage: React.FC = () => {
  ...
  // 5. Group deviations by SRF
  const groupedBySRF = useMemo(() => {
    const groups: Record<string, PaginatedDeviationItem[]> = {};
 
    items.forEach((item) => {
      const srfKey = item.srf_no || "Unknown SRF";
      if (!groups[srfKey]) {
        groups[srfKey] = [];
      }
      groups[srfKey].push(item);
    });
 
    // Sort SRF keys alphabetically
    const sortedGroups: Record<string, PaginatedDeviationItem[]> = {};
    Object.keys(groups)
      .sort()
      .forEach((key) => {
  ...
  // Navigate to SRF equipment page, passing data via state
  const handleSRFClick = (srfKey: string, deviations: PaginatedDeviationItem[]) => {
    navigate(`/engineer/deviations/srf/${encodeURIComponent(srfKey)}`, {
      state: { deviations, activeTab },
    });
  };
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden relative z-0">
          ...
          <div className="bg-gray-50 border-b border-gray-200 flex space-x-1 overflow-x-auto px-2 pt-2 shrink-0">
            <button
              ...
              className={`flex items-center gap-2 px-5 py-3 font-medium text-sm rounded-t-lg transition-all ${
                activeTab === "OOT"
                  ? "bg-white border-t border-x border-gray-200 shadow-sm text-red-600"
                  : "text-gray-500 hover:bg-gray-200"
              }`}
            >
              Deviation - OOT
              {activeTab === "OOT" && (
                <span className="ml-1 px-2 py-0.5 rounded-full text-xs font-bold bg-gray-100 text-gray-800">
                  {totalRecords}
                </span>
              )}
            </button>
            <button
              ...
              className={`flex items-center gap-2 px-5 py-3 font-medium text-sm rounded-t-lg transition-all ${
                activeTab === "MANUAL"
                  ? "bg-white border-t border-x border-gray-200 shadow-sm text-gray-800"
                  : "text-gray-500 hover:bg-gray-200"
              }`}
            >
              Deviation - NC (Manual)
          ...
          {/* Table Body Area - SRF Cards Only */}
          <div className="relative min-h-[400px] bg-white p-4 sm:p-6">
            ...
            {Object.keys(groupedBySRF).length === 0 && !isFetchingData && !error ? (
              ...
              <div className="space-y-4">
                {Object.entries(groupedBySRF).map(([srfKey, deviations]) => {
                  const equipmentCount = deviations.length;
 
                  return (
                    <div
                      key={srfKey}
                      onClick={() => handleSRFClick(srfKey, deviations)}
                    ...
                    >
                      <div className="flex items-start gap-4">
                        <div className="mt-1 p-2.5 rounded-full bg-red-50 text-red-600 transition-colors group-hover:bg-red-100">
                          <AlertTriangle size={20} />
                        </div>
                        <div>
                          <p className="font-bold text-xl text-gray-800 group-hover:text-blue-700 transition-colors">
                            SRF: {srfKey}
                          </p>
                          <p className="text-sm text-gray-500 mt-1 font-medium">
                            {equipmentCount} equipment
                            {equipmentCount > 1 ? "s" : ""} with deviations
                          </p>
                          <div className="flex items-center gap-2 mt-2">
                            <span className="text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider border bg-blue-50 text-blue-700 border-blue-200">
                              {activeTab}
                            </span>
                          </div>
                        </div>
...
// ============================================================
// SRF EQUIPMENT PAGE - Shows equipment under selected SRF
// ============================================================
export const DeviationSRFEquipmentPage: React.FC = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { srfNo } = useParams<{ srfNo: string }>();
 
  const decodedSrfNo = decodeURIComponent(srfNo || "");
 
  // Try to read data passed via navigation state (no refetch needed)
  const stateData = (location.state as {
    deviations?: PaginatedDeviationItem[];
    activeTab?: "OOT" | "MANUAL";
  } | null) || null;
 
  const [items, setItems] = useState<PaginatedDeviationItem[]>(
    stateData?.deviations || []
  );
  const [loading, setLoading] = useState(!stateData?.deviations);
  const [error, setError] = useState<string | null>(null);
 
  useEffect(() => {
    // If we already have data from navigation state, skip fetching
    if (stateData?.deviations && stateData.deviations.length > 0) {
      setItems(stateData.deviations);
      setLoading(false);
      return;
    }
 
    // Fallback: fetch all deviations and filter (e.g. on page refresh)
    const loadEquipment = async () => {
      try {
        setLoading(true);
        setError(null);
 
        // Fetch a large page so all SRF equipment is included
        const res = await api.get<any>(ALL_STAFF_DEVIATIONS_ENDPOINT, {
          params: { skip: 0, limit: 1000 },
        });
 
        const allItems: PaginatedDeviationItem[] = res.data.items || [];
        const filtered = allItems.filter(
          (i) => i.srf_no === decodedSrfNo
        );
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-900 tracking-tight">
                SRF: {decodedSrfNo || "Unknown"}
              </h2>
              <p className="text-gray-500 text-sm mt-1">
                Equipment with deviations
              </p>
            </div>
          </div>
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 font-medium text-sm shadow-sm transition-colors"
          >
            <ChevronLeft size={16} /> Back to SRF List
          </button>
        </div>
          ...
          <div className="p-6">
            ...
            {!loading && !error && items.length === 0 && (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-gray-500 mt-1 max-w-sm mx-auto">
                  No deviations found for this SRF.
                </p>
              </div>
            )}
              ...
              <div className="space-y-3">
                {items.map((item) => (
                  ...
                  >
                    <div className="flex items-start gap-4">
                      <div
                        className={`mt-1 p-2.5 rounded-full transition-colors ${
    ...
    const loadDetail = async () => {
      ...
      try {
        ...
        if (id < 0) {
          ...
          res = await api.get<DeviationDetailResponse>(
            `/external-deviations/${Math.abs(id)}`
          );
        } else {
          // It's an Internal Deviation
          res = await api.get<DeviationDetailResponse>(
            ENDPOINTS.STAFF_DEVIATIONS.DETAIL(id)
          );
        }
          ...
          <div className="space-y-5 text-sm">
                ...
                <div className="mt-1 flex items-center gap-2">
                  <div
                    className={`h-2 w-2 rounded-full ${
                      isCurrentlyVisible() ? "bg-emerald-500 animate-pulse" : "bg-slate-400"
                    }`}
                  />
              ...
              <div className="flex flex-wrap gap-3">
                ...
                {!isExternalRecord && (
                  <>
                    <button
                      disabled={
                        closingDeviation || (detail.status || "").toUpperCase() === "CLOSED"
                      }
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                ...
                <span
                  className={`inline-flex text-xs px-3 py-1 rounded-full font-bold border tracking-wide uppercase ${
                    (detail.status || "").toUpperCase() === "CLOSED"
              ...
              <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                ...
                <span
                  className={`inline-flex text-xs px-3 py-1 rounded-full font-bold border tracking-wide ${
                    (detail.tool_status || detail.calibration_status || "")
                      .toLowerCase()
                      .includes("calibrated") ||
                    (detail.tool_status || detail.calibration_status || "")
                      .toLowerCase()
                ...
                >
                  {formatCalibrationStatus(
                    isExternalRecord ? detail.tool_status : detail.calibration_status
                  )}
                </span>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <div>
                  <span className="text-gray-500 font-medium">SRF</span>
                  <br />
                ...
                <div>
                  <span className="text-gray-500 font-medium">NEPL ID</span>
                  <br />
                ...
                <div>
                  <span className="text-gray-500 font-medium">Report date</span>
                  <br />
                  <span className="font-bold text-gray-900 text-base">
                    {detail.report ? formatDcDate(detail.report) : "—"}
                  </span>
                </div>
              </div>
              ...
              {detail.deviation_type === "OOT" && (detail.oot_steps?.length || 0) > 0 && (
                <div className="mt-6 border border-gray-200 rounded-xl overflow-hidden">
                  <div className="bg-gray-50 px-4 py-2 border-b border-gray-200">
                    <p className="text-gray-700 text-xs font-bold uppercase tracking-wide">
                      OOT Steps
                    </p>
                  </div>
            ...
            <div className="rounded-2xl border border-blue-100 bg-blue-50/30 p-5 shadow-sm">
              <p className="text-xs uppercase tracking-wide text-blue-800 font-bold mb-2">
                Engineer remarks
              </p>
              <textarea
              ...
              <div className="pt-3 flex justify-end">
                ...
                >
                  {savingRemarks ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save size={16} />}{" "}
                  Save remarks
                </button>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 shadow-sm">
              <p className="text-xs uppercase tracking-wide text-amber-800 font-bold mb-2">
                Customer decision
              </p>
              <div className="text-gray-800 font-medium whitespace-pre-wrap bg-white border border-amber-100 rounded-xl p-4 min-h-[60px] shadow-sm">
                {detail.customer_decision || (
                  <span className="text-gray-400 italic">No decision provided yet.</span>
                )}
              </div>
            </div>
                    ...
                    >
                      <div className="flex items-center gap-3 overflow-hidden">
                        ...
                        <div className="flex flex-col overflow-hidden">
                          <span className="text-sm font-bold text-gray-800 truncate group-hover:text-blue-700">
                            {a.file_name}
                          </span>
                          {a.file_type && (
