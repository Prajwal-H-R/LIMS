...
// HELPERS
// ─────────────────────────────────────────────────────────────────────────────
const LOADING_POSITIONS = [-10, 10] as const;

const buildDefaultRows = (): LoadingRowData[] =>
  LOADING_POSITIONS.map((mm) => ({
...
const calcBL = (rows: LoadingRowData[]): number => {
  const m1 = rows[0]?.mean_value;
  const m2 = rows[1]?.mean_value;
  return m1 !== null && m2 !== null && m1 !== undefined && m2 !== undefined
    ? Math.abs(m1 - m2)
    : 0;
};
...
// ─────────────────────────────────────────────────────────────────────────────
const LoadingPointSection = forwardRef<
  SectionBotHandle,
  LoadingPointSectionProps
>(({ jobId }, ref) => {
  // ── State ──────────────────────────────────────────────────────────────────
  const [loading, setLoading] = useState(false);
  const [dataLoaded, setDataLoaded] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.error("LoadingPoint fetch failed:", err);
      } finally {
        setLoading(false);
      }
    };
...
LoadingPointSection.displayName = "LoadingPointSection";
export default LoadingPointSection;